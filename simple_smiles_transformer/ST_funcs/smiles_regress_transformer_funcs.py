...
def stratified_sample(data, y, bin_left, bin_right, numbins):
    ...
    x_train, x_test, y_train, y_test = train_test_split(data, y, test_size=0.3, stratify=y_binned)
...
def train_val_data(hyper_params):
    ...
    if hyper_params['data_loading']['stratified']:
        ...
        data['DockingScore'] = -1* data['DockingScore']
        data = data[:10000]
        print(data.describe())
        y = data['DockingScore']
        ...
        data_train, data_vali = stratified_sample(data, y, bin_left, bin_right, (bin_right - bin_left))
    ...
    y_train = data_train["DockingScore"].values.reshape(-1, 1) * 1.0 
    y_val = data_vali["DockingScore"].values.reshape(-1, 1) * 1.0
